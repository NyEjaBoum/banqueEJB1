...
public class CompteDepotClient {
    ...
    public String creerCompte(int clientId, Double plafondRetrait, Double tauxInteret) throws Exception {
        StringBuilder url = new StringBuilder(BASE_URL + "creer?clientId=" + clientId);
            if (plafondRetrait != null) {
                url.append("&plafondRetrait=").append(plafondRetrait);
            }
            if (tauxInteret != null) {
                url.append("&tauxInteret=").append(tauxInteret);
            }
    ...
    public String appliquerInteret(int compteId) throws Exception {
        String url = BASE_URL + "interet?compteId=" + compteId;
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(url))
            .POST(HttpRequest.BodyPublishers.noBody())
